...
function 下载bat(域名,token) {
	return [
	  ...
	  `for /f "delims=" %%i in ('powershell -command "$content = ((Get-Content -Path '%cd%/%FILENAME%' -Encoding UTF8) | Select-Object -First 300) -join [Environment]::NewLine; [convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($content))"') do set "BASE64_TEXT=%%i"`,
	  ``,
...
BASE64_TEXT=$(head -n 300 $FILENAME | base64 -w 0)
	...
	return `
	  <html>
		...
		<body>
		  ...
		  token: ${token} <br>
		  <br>
		  <pre>注意! 因URL长度内容所限，脚本更新方式一次最多更新300行内容</pre><br>
